"""Evaluation for real_full ViLT + Flickr30K multimodal verification pipeline."""
...
from src.models.vilt_model import ViltMultimodalEmbedder, resolve_vilt_device
...
def evaluate_real_full_vilt_flickr30k(config: Dict, run_dir: Path) -> Dict[str, float | int | str]:
    ...
    triggers = load_multimodal_triggers_from_manifest(
        manifest_path,
        csv_key="flickr30k_vilt_triggers_csv",
        json_key="flickr30k_vilt_triggers_json",
        default_csv_path="data/processed/triggers/multimodal/flickr30k_vilt_triggers.csv",
    )

    device = resolve_vilt_device(training.get("device", "auto"))
    model_cfg = config.get("model", {})
    backend = str(model_cfg.get("backend", "transformers_vilt"))
    model_name = str(model_cfg.get("name", "dandelin/vilt-b32-mlm"))
    ...
    return {
        ...
        "embedding_mode": str(config.get("watermark", {}).get("embedding_mode", "vilt_pooler_output_normalized")),
        "backend": backend,
